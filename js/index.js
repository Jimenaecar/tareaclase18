...
function filtrarAutor(arreglo, autor) {
    ...
    const resultado = arreglo.filter((item) => item.autor === autor);
    return resultado;
...
function mostrarFiltrados() {
    ...
    let presentarLibros = document.getElementById("libros");
    const autoresFiltrados = filtrarAutor(libros, valueAutores.value);

    presentarLibros.innerHTML = "";

    autoresFiltrados.forEach((item) => {
        presentarLibros.innerHTML += `<div class="card" style="width: 18rem;">
